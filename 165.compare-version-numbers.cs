...
public class Solution {
    ...
    {
        double v1 = ParseVersion(version1);
        double v2 = ParseVersion(version2);
    ...
    private double ParseVersion(string version)
    {
        double r = 0;
        double dig = 1;
        string[] ps = version.Split('.');
        for (int i = 0; i < ps.Length; i++)
        {
            r += int.Parse(ps[i]) * dig;
            dig *= 0.1d;
        }
...
// √ Accepted
//   √ 72/72 cases passed (76 ms)
//   √ Your runtime beats 95.96 % of csharp submissions
//   √ Your memory usage beats 5.88 % of csharp submissions (20.4 MB)
