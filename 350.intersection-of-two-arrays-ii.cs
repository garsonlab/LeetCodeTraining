/*
 * @lc app=leetcode id=350 lang=csharp
 *
 * [350] Intersection of Two Arrays II
 *
 * https://leetcode.com/problems/intersection-of-two-arrays-ii/description/
 *
 * algorithms
 * Easy (46.96%)
 * Total Accepted:    186.4K
 * Total Submissions: 396K
 * Testcase Example:  '[1,2,2,1]\n[2,2]'
 *
 * Given two arrays, write a function to compute their intersection.
 * 
 * Example 1:
 * 
 * 
 * Input: nums1 = [1,2,2,1], nums2 = [2,2]
 * Output: [2,2]
 * 
 * 
 * 
 * Example 2:
 * 
 * 
 * Input: nums1 = [4,9,5], nums2 = [9,4,9,8,4]
 * Output: [4,9]
 * 
 * 
 * Note:
 * 
 * 
 * Each element in the result should appear as many times as it shows in both
 * arrays.
 * The result can be in any order.
 * 
 * 
 * Follow up:
 * 
 * 
 * What if the given array is already sorted? How would you optimize your
 * algorithm?
 * What if nums1's size is small compared to nums2's size? Which algorithm is
 * better?
 * What if elements of nums2 are stored on disk, and the memory is limited such
 * that you cannot load all elements into the memory at once?
 * 
 * 
 */
public class Solution {
    public int[] Intersect(int[] nums1, int[] nums2) {
        Dictionary<int, int> marks = new Dictionary<int, int>();
        for (int i = 0; i < nums1.Length; i++)
        {
            int n = 0;
            if(!marks.TryGetValue(nums1[i], out n)){
                n = 0;
            }
            marks[nums1[i]] = n+1;
        }

        List<int> inters = new List<int>();
        for (int i = 0; i < nums2.Length; i++)
        {
            int n = 0;
            if(marks.TryGetValue(nums2[i], out n)){
                inters.Add(nums2[i]);

                if(n <= 1)
                    marks.Remove(nums2[i]);
                else
                    marks[nums2[i]] = n-1;
            }
        }
        return inters.ToArray();
    }
}


// √ Accepted
//   √ 61/61 cases passed (248 ms)
//   √ Your runtime beats 95.5 % of csharp submissions
//   √ Your memory usage beats 34.48 % of csharp submissions (29.3 MB)

