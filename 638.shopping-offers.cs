...
public class Solution {
    public int ShoppingOffers(IList<int> price, IList<IList<int>> special, IList<int> needs)
    {
        ...
        for (int i = 0; i < special.Count; i++)
        {
            ...
            {
                List<int> remainNeeds = new List<int>();
                for (int j = 0; j < needs.Count; j++)
                {
                    //剩余需求商品
                    remainNeeds.Add(needs[j] - special[i][j]);
                }
                //当前花费=递归下一层花费+本次礼包花费
                int tempPrice = ShoppingOffers(price, special, remainNeeds) + special[i][needs.Count];
                //比较当前花费与直接购买商品花费对比，得到当前递归层最低消费
                res = Math.Min(res, tempPrice);
            }
        }
    ...
    private bool CheckValid(IList<int> special, IList<int> needs)
    {
